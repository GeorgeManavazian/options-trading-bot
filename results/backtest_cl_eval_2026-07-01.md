...
**Data:** Databento CME `GLBX.MDP3`, `CL.c.0` continuous front-month, **1-min + daily, 2010-06-07 → 2026-06-26** (3,969 trading days). Raw output: `results/backtest_cl_raw_2026-07-01.txt`; grade JSON/MD: `results/backtest_eval_2026-07-01_155042.*`. (Numbers below are post-roll-fix — see caveat #4.)
...
- Best config = **same-day hold** (enter at the signal's intraday level, exit same-day close): **+1043% on risk, 64% win, risk-adj +31.73, PF 2.19**, positive in **16 of 17 calendar years** (only partial-2026 negative), survives slippage to **5 ticks/side** (+635%).

## The breakout result (the reason we did this)

| Family | n | win | avg win | avg loss | PF | same-day total | +5d total |
|---|---|---|---|---|---|---|---|
| **Breakouts** | 1112 | 54.9% | 1.07% | 0.96% | **1.36** | **+174%** | **+444%†** |
| Fades | 1442 | 70.7% | 1.24% | 0.95% | **3.18** | +869% | +774%† |

† The +5d column crosses contract rolls on the raw continuous series — indicative only (see caveat #4). Same-day columns are clean.
...
4. **Raw continuous contract + rolls.** `CL.c.0` is raw front-month (Databento does not back-adjust). Roll days (instrument_id change; ~193 over 16 yrs) emit no signals AND are kept in the day sequence so the *following* day's pivot uses its correct same-contract prior — a review-caught off-by-one that was fixed (post-fix the same-day headline moved only +1044→+1043%, confirming the ~4% of affected days were immaterial). **Same-day results are fully within-contract and clean.** BUT the multi-day tables (+1d/+3d/+5d — both the forward-return and P&L rows) can still embed the raw roll gap when a holding window spans a roll, so treat all multi-day numbers as *indicative only*. The recommended same-day config is unaffected.
